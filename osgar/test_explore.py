...
#        self.assertIsNone(angle)  # no tangent
    ...
    def test_follow_wall_angle(self):
        ...
        cmd = follow_wall_angle(scan, gap_size=1.0, wall_dist=1.0, right_wall=True)
        ...
        # move towards wall
        cmd = follow_wall_angle(scan, gap_size=0.7, wall_dist=0.7, right_wall=True)
        self.assertIsNotNone(cmd)
        self.assertLess(math.degrees(cmd), 0)  # used to be -45, now only slightly move right

        # move from wall
        cmd = follow_wall_angle(scan, gap_size=1.3, wall_dist=1.3, right_wall=True)
        self.assertIsNotNone(cmd)
