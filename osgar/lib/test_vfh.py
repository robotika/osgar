...
def createVFH(verbose=False):
    ...
    return VectorFieldHistogram(robot, laserFOV=fov, blockingDistance=0.,
        safetyDistance=0.10, maxRange=1.5, turnRadius=0.5, binSize=binSize,
        verbose=verbose)


class VectorFieldHistogramTest(unittest.TestCase):
    def test_usage(self):
        robot = PseudoRobot()
        fov = math.radians(FOV_DEGS)
        vfh = createVFH()
