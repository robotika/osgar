...
do
    ...
    do
        ...
        timeout $TIMEOUT sh -c "( docker run --rm --network=host -t --name nasa-rover rover:latest /osgar/moon/docker/rover/run_solution.bash -u -r $ROUND &) 2>&1 | tee -a \"${log_file}\" | grep -m 1 ${FAIL_SWITCH} -e \"${SUCCESS_STRING}\""
