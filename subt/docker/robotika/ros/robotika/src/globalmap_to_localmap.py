...
def updateMap():
    global map_pub, lastMap, lastPose, transformListener, robotName
    now = rospy.Time.now()
    try:
    ...
    newMap.header.frame_id = robotName + '/odom'
...
if __name__ == '__main__':
    try:
        ...
        map_pub = rospy.Publisher('/' + robotName + '/local_map', OccupancyGrid)
        rospy.Subscriber('/rtabmap/grid_map',OccupancyGrid, mapCallback)
        rospy.Subscriber('/odometry/filtered', Odometry, odomCallback, queue_size=15)
