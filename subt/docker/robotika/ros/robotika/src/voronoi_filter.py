...
CURRENT_VERTEX_THRESHOLD = 0.4
NEAR_VERTEX_THRESHOLD = 1.7
...
def getMarkerForVertex(vertex,size):
    global markerId
    marker = Marker()
...
def segmentsCallback(segments):
    ...
    for vertex in segments.vertices:
        if vertex.width <= MIN_VERTEX_WIDTH:
            continue
        vertexPose = Object()
        vertexPose.x = vertex.path[0].x + lastMap.info.origin.position.x
        vertexPose.y = vertex.path[0].y + lastMap.info.origin.position.y
        newSegments.append(vertexPose) 
...
def stateGenerator():
    ...
    while True:
        ...
        if robomath.distPose(plannedGoal,lastPose) <= GOAL_REACHED_THRESHOLD:
            ...
            for vertex in lastSegments:
                markerArray.markers.append(getMarkerForVertex(vertex,0.1))
...
if __name__ == '__main__':
    try:
        ...
        baseLinkFrame = "X0F40L" #rospy.get_param('~base_link_frame')
