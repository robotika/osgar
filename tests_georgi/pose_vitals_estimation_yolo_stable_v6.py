...
class TrackedPerson:
    def __init__(self, track_id, fps):
        ...
        self.vitals = {"hr_bpm": "Calculating...", "breathing": "Calculating..."}
...
def update_heart_rate(frame, keypoints, person):
    ...
    # --- Selectively apply CLAHE only on the face ROI ---
    ...
    # Use the enhanced ROI for the rPPG signal
    person.rgb_buffer_hr.append(np.mean(enhanced_face_roi, axis=(0, 1)))

    if len(person.rgb_buffer_hr) < person.hr_buffer_size or person.vitals_update_counter != 0:
        return "PROCESSING"
...
def is_breathing_signal_plausible(person):
    """
    Checks if the breathing signals are physiologically plausible.
    """
    if len(person.breathing_area_signal) < person.breathing_buffer_size:
        return True  # Not enough data yet, don't invalidate

    # Detrend signals to focus on oscillations
    ...
    # 1. Magnitude Check: Is the movement significant enough?
    if np.std(detrended_area) < Config.BREATHING_MIN_SIGNAL_STD:
        return False  # Movement is too small, likely just noise

    # 2. Correlation Check: Does it move like a real chest?
    # Chest area should increase as shoulders rise (y decreases). They must be negatively correlated.
    correlation_matrix = np.corrcoef(detrended_area, detrended_shoulder_y)
    correlation = correlation_matrix[0, 1]

    if correlation > Config.BREATHING_CORRELATION_THRESHOLD:
        return False  # Movement is not correlated correctly for breathing
...
def update_breathing_rate(keypoints, person):
    ...
    if not is_breathing_signal_plausible(person):
        ...
        # Reset history to prevent stale data from being used later
...
def calculate_robust_motion_score(roi1, roi2, motion_thresh_val=25):
    """Calculates a motion score robust to lighting changes and noise."""
...
def update_liveness_status(person, hr_status, current_frame_idx):
    """
    A stable, state-based liveness detection logic.
    """
    # Define success and failure events for this frame
    ...
    # Update the reading history with a decay factor to prioritize recent events
    reading_decay = 0.98
    ...
    if is_success:
        ...
        person.failed_readings += 0.5  # A low signal is a minor failure
    elif is_no_signal:
        person.failed_readings += 1  # A complete lack of signal is a major failure

    # Calculate a stable liveness score based on the history of readings
    total_readings = person.successful_readings + person.failed_readings
    liveness_score = 0.0
    if total_readings > 1:  # Require a minimum number of readings to start
        liveness_score = person.successful_readings / total_readings

    # Determine the primary status based on the score
    if liveness_score > Config.LIVENESS_ALIVE_THRESHOLD:
        ...
        # Only declare "No Vitals" after several attempts
        new_status = "No Vitals Detected"
    else:
        new_status = "Determining..."

    # Secondary Check: Refine "Alive" status with heart rate details
    if new_status == "Alive" and "BPM" in str(person.vitals["hr_bpm"]):
        hr_value = float(re.search(r"(\d+\.\d+)", person.vitals["hr_bpm"]).group(1))
        if hr_value < Config.HR_MIN_BPM_FOR_ALIVE:
            new_status = "Alive (Low HR)"  # Modify status, but it's still "Alive"

    person.liveness_status = new_status

    # Timer Logic: triggers for any "Alive" state
    if "Alive" in person.liveness_status:
        ...
        # If status is not a form of "Alive", reset the timer
...
def run_pose_and_vitals_inference(args):
    ...
    if fps < 10: fps = 30  # Fallback for incorrect metadata
    ...
    try:
        for frame_idx in tqdm(range(total_frames), desc="Processing Video"):
            ...
            for track_id, person in track_history.items():
                ...
                # --- Dynamically build the text panel ---
                liveness_score = person.successful_readings / (person.successful_readings + person.failed_readings + 0.000001)
                text_lines = [
                    f"ID {track_id} (Conf: {liveness_score:.2f})",
                    f"Status: {status}",
                ]
                # Add the duration string only if it's available
                if person.alive_duration_str:
                    text_lines.append(person.alive_duration_str)

                text_lines.extend([
                    f"Breathing: {person.vitals['breathing']}",
                    f"Heart Rate: {person.vitals['hr_bpm']}"
                ])
                ...
                for i, line in enumerate(text_lines):
                    ...
                    if "Alive for:" in line:
                        color = (150, 255, 150)  # Light green for timer
                    elif "Alive" in line:
                        color = (0, 255, 0)  # Green
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model_file', type=str, default="yolo11x-pose.pt",
                        help='Path to the YOLO pose model file (e.g., yolo11s-pose.pt for faster performance).')
    ...
    parser.add_argument('--motion_thresh', type=float, default=0.01,
                        help='Motion detection threshold (fraction of changed pixels). Increase if static people are missed, decrease if moving mannequins are processed.')
